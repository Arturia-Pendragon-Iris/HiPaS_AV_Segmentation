...
from HiPaS.train.unet3d.utils import expand_as_one_hot
...
class DiceLoss(_AbstractDiceLoss):
    ...
    def dice(self, input, target, weight):
        ...
        dice = compute_per_channel_dice(input, target, weight=self.weight)
        # print("dice is")
        # print(dice)
        return dice
...
# def soft_sigmoid_dice(y_true, y_pred):
#     """[function to compute dice loss]
#
#     Args:
#         y_true ([float32]): [ground truth image]
...
#     intersection = torch.sum((y_true * p))
#     coeff = (2. * intersection + smooth) \
#             / (torch.sum(y_true) + torch.sum(p) + smooth)
#     return coeff


class SoftDiceclDice(nn.Module):
    def __init__(self, iter_=3, alpha=0.5, smooth=0.0001):
    ...
    def forward(self, input, target):
        dice_loss = self.dice(input[0], target)
        input = self.normalization(input[0])
        # print(torch.max(input), torch.min(input))
        # for i in range(input.shape[1]):
        #     dice_loss = 1.0 - soft_dice(input[:, i], target[:, i]) / input.shape[1]
        # print('dice loss: {:.3f}'.format(dice_loss))
        skel_pred = soft_skel(input, self.iter)
        skel_true = soft_skel(target, self.iter)
        # cl_dice_loss = 1 - (torch.sum(torch.multiply(skel_pred, skel_true)) + self.smooth)/\
        #         (torch.sum(skel_pred) + torch.sum(skel_true) + self.smooth)
        # # print(torch.sum(torch.multiply(skel_pred, target)))
        # # print(torch.sum(skel_pred))
        # # print(cl_dice_loss)
        tprec = (torch.sum(torch.multiply(skel_pred, target)) + self.smooth)/\
                (torch.sum(skel_pred) + self.smooth)
        tsens = (torch.sum(torch.multiply(skel_true, input)) + self.smooth)/\
                (torch.sum(skel_true) + self.smooth)
        # print(tprec, tsens)
        cl_dice_loss = 1.0 - 2.0 * (tprec * tsens) / (tprec + tsens)
        # print(dice_loss, cl_dice_loss)
        # print(cl_dice_loss, dice_loss)
        return (1.0 - self.alpha) * dice_loss + self.alpha * cl_dice_loss
        # return dice_loss


class SoftDice_deep(nn.Module):
    def __init__(self):
        super(SoftDice_deep, self).__init__()
        self.loss = DiceLoss()

    def forward(self, input, target):
        # print(input.shape, target.shape)
        loss = self.loss(input[0], target)
        # for sub_index in range(1, 5):
        #     sub_mask = F.interpolate(target, scale_factor=2 ** (-1 * sub_index))
        #     # print(sub_mask.shape, input[sub_index].shape)
        #     loss += self.loss(input[sub_index], sub_mask).mean() * 0.1
        return loss
...
def _create_loss(name, loss_config, weight, ignore_index, pos_weight):
    if name == 'BCEWithLogitsLoss':
        ...
        return BCEDiceLoss()
    elif name == 'SoftDiceclDice':
        return SoftDiceclDice()
    elif name == 'SoftDice_deep':
        return SoftDice_deep()
