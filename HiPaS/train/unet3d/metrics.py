...
from HiPaS.train.unet3d.losses import compute_per_channel_dice
from HiPaS.train.unet3d.seg_metrics import AveragePrecision, Accuracy
from HiPaS.train.unet3d.utils import get_logger, expand_as_one_hot, convert_to_numpy
...
class Sensitivity:
    ...
    def __call__(self, input, target):
        ...
        for k in range(input.shape[1]):
            pred = input[:, k].flatten()
            label = target[:, k].flatten()
            fn = torch.sum(label - pred * label)
            total_fnr += fn / (torch.sum(label))
        return 1 - total_fnr / input.shape[1]
...
def get_evaluation_metric(config):
    ...
    def _metric_class(class_name):
        m = importlib.import_module('HiPaS.train.unet3d.metrics')
