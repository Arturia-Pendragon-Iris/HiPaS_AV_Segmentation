...
from HiPaS.train.datasets.utils import get_train_loaders
from HiPaS.train.unet3d.losses import get_loss_criterion
from HiPaS.train.unet3d.metrics import get_evaluation_metric
from HiPaS.train.unet3d.utils import get_logger, get_tensorboard_formatter, create_optimizer, \
...
def create_trainer(config):
    model = HiPaSNet(in_channels=2, out_channels=2, mid_channels=24)
...
class UNetTrainer:
    ...
    def train(self):
        ...
        for t in self.loaders['train']:
            ...
            ct_data, target, weight = self._split_training_batch(t)
            ...
            if self.num_iterations % self.log_after_iters == 0:
                save_checkpoint(model=self.model, epoch=self.num_epochs)
    ...
    def _forward_pass(self, input, target, weight=None):
        output = self.model(input[:, :2], input[:, 2:])
...
def save_checkpoint(model, epoch):
    model_out_path = "/data/Train_and_Test/Artery_Vein/checkpoint/" \
                     + "med24_epoch_{}.pth".format(epoch)
