...
def load_checkpoint(checkpoint_path, model, optimizer=None,
    ...
    state = torch.load(checkpoint_path, map_location='cpu')['model_state_dict']

    model_dict = model.state_dict()
    pretrained_dict = {k: v for k, v in state.items() if
                       (k in model_dict)}
...
def get_tensorboard_formatter(formatter_config):
    ...
    m = importlib.import_module('HiPaS.train.unet3d.utils')
